...
class GF_RECAPTCHA extends GFAddOn {
	...
	public function pre_init() {
		...
		if ( class_exists( 'Gravity_Forms\Gravity_Forms_Geolocation\GF_Geolocation' ) ) {
			$r              = \Gravity_Forms\Gravity_Forms_Geolocation\GF_Geolocation::get_instance();
			$google_api_key = $r->get_google_places_api_key();

			if ( ! empty( $google_api_key ) ) {
				// This is what matters.
				define( 'GF_GEOLOCATION_GOOGLE_API_KEY', $google_api_key );
			} else {
				wp_die( 'vaya' );
			}
		} else {
		...
		$this->api   = \GF_GEOLOCATION_GOOGLE_API_KEY;
